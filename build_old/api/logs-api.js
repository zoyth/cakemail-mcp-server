...
export class LogsApi extends BaseApiClient {
    ...
    async getCampaignLogs(campaignId, params) {
        ...
        const buildEndpoint = (basePath) => {
            ...
            if (params?.with_count !== undefined)
                url.searchParams.set('with_count', params.with_count.toString());
            if (params?.sort)
                url.searchParams.set('sort', params.sort);
            if (params?.order)
                url.searchParams.set('order', params.order);
            if (params?.start_time)
                url.searchParams.set('start_time', params.start_time.toString());
            if (params?.end_time)
                url.searchParams.set('end_time', params.end_time.toString());
            if (params?.cursor)
    ...
    hasEngagementData(result) {
        ...
        const engagementIndicators = [
            // Event type patterns
            (log) => log.type && ['click', 'open', 'bounce', 'unsubscribe', 'spam'].some(type => log.type.toLowerCase().includes(type)),
            // Has clickthrough URL (indicates click tracking)
            (log) => log.clickthru_url !== null && log.clickthru_url !== undefined,
            // Has event_type field with engagement values
            (log) => log.event_type && ['click', 'open', 'bounce', 'unsubscribe'].includes(log.event_type.toLowerCase()),
            // Has activity_type field
            (log) => log.activity_type && log.activity_type !== 'delivery',
            // Non-delivery type events
            (log) => log.type && !['generating', 'in_queue', 'received', 'sent', 'delivered'].includes(log.type.toLowerCase())
        ];
        // Check if any log entry matches engagement patterns
        const hasEngagement = result.data.some((log) => engagementIndicators.some(indicator => indicator(log)));
     ...
     */
    async getWorkflowActionLogs(workflowId, actionId, params) {
     ...
     */
    async getWorkflowLogs(workflowId, params) {
     ...
     */
    async getTransactionalEmailLogs(params) {
        ...
        if (params?.email_id)
            url.searchParams.set('email_id', params.email_id);
        if (params?.sender_id)
            url.searchParams.set('sender_id', params.sender_id);
        if (params?.status)
    ...
    analyzeLogs(logResult) {
        ...
        logResult.data.forEach((log, index) => {
            // Collect event types
            if (log.type)
                typeSet.add(log.type);
            if (log.event_type)
                typeSet.add(log.event_type);
            if (log.activity_type)
                typeSet.add(log.activity_type);
     ...
     */
    validateFilter(filter, endpoint) {
     ...
     */
    getEventsInCategory(categoryName) {
     ...
     */
    analyzeLogsWithTaxonomy(logResult) {
