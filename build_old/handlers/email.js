 ...
 */
export async function handleSendEmail(args, api) {
    try {
        const { to_email, to_name, sender_id, subject, html_content, text_content, template_id, list_id, email_type = 'transactional' } = args;
 ...
 */
export async function handleGetEmail(args, api) {
    try {
        const { email_id } = args;
        if (!email_id) {
 ...
 */
export async function handleRenderEmail(args, api) {
    try {
        const { email_id, as_submitted = false, tracking = false } = args;
        if (!email_id) {
            throw new EmailAPIError('email_id is required', 400);
        }
 ...
 */
export async function handleGetEmailLogs(args, api) {
    try {
        const { log_type = 'all', email_id, iso_time = false, page = 1, per_page = 50, start_time, end_time, tags, providers, sort = '-time' } = args;
        const options = {
            log_type,
            iso_time,
            page,
            ...
            content: [
                {
                    ...
                    text: `📋 **Email Logs Retrieved**\n\n` +
                        ...
                        (displayLogs.map((log, i) => `${i + 1}. **${log.type}** - Email ID: ${log.email_id} (${log.time})`).join('\n') || 'No logs found.') +
...
export async function handleGetEmailStats(args, api) {
    try {
        const { interval = 'day', iso_time = false, start_time, end_time, providers, tags } = args;
        const options = {
            interval,
            iso_time
        };
            ...
            content: [
                {
                    ...
                    text: `📊 **Email Statistics Retrieved**\n\n` +
                        ...
                        (statsEntries.map((entry, i) => `${i + 1}. Time: ${entry.time} | Delivered: ${entry.delivered || 0} | Opened: ${entry.open || 0}`).join('\n') || 'No statistics found.') +
 ...
 */
export async function handleSendTransactionalEmail(args, api) {
    try {
 ...
 */
export async function handleSendMarketingEmail(args, api) {
    try {
 ...
 */
export async function handleGetEmailLogsWithAnalysis(args, api) {
    try {
        const { log_type = 'all', email_id, iso_time = false, page = 1, per_page = 50, start_time, end_time, tags, providers, sort = '-time' } = args;
        const options = {
            log_type,
 ...
 */
export async function handleDebugEmailAccess(args, api) {
    try {
        const { email_id } = args;
        let result = {
