...
import { validateBEETemplate as validateBEETemplateUtil, printBEETemplateStructure as printBEETemplateStructureUtil } from '../utils/bee-editor.js';
export async function handleListCampaigns(args, api) {
    try {
        let { page, per_page, status, name, type, list_id, sort, order, with_count, account_id } = args;
...
export async function handleGetLatestCampaigns(args, api) {
    try {
        ...
        const campaignList = campaigns.data?.map((campaign) => ({
            id: campaign.id,
            name: campaign.name,
            subject: campaign.subject,
            status: campaign.status,
            ...
            content: [
                {
                    ...
                    text: `📧 **Latest ${count} Campaigns**\n\n` +
                        ...
                        (campaignList?.map((camp, i) => `${i + 1}. **${camp.name}** (${camp.id})\n` +
                            ...
                            `   🏷️ Status: ${camp.status || 'N/A'}\n` +
                            `   📂 Type: ${camp.type || 'N/A'}\n` +
                            `   📅 Created: ${camp.created_on || 'N/A'}`).join('\n\n') || 'No campaigns found.') +
                        `\n\n**Full Response:**\n${JSON.stringify(campaigns, null, 2)}`,
                },
            ],
        };
...
export async function handleGetCampaign(args, api) {
    try {
        ...
        const campaignData = campaign.data; // Use any to access extended properties
...
export async function handleCreateCampaign(args, api) {
    try {
        const { name, audience, // { list_id: number, segment_id?: number }
        ...
        reply_to_email, account_id // Agency/Enterprise account scoping
         } = args;
...
export async function handleUpdateCampaign(args, api) {
    try {
        const { campaign_id, name, audience, // { list_id: number, segment_id?: number }
        sender, // { id: string, name?: string }
...
export async function handleSendCampaign(args, api) {
    try {
        const { campaign_id } = args;
        if (!campaign_id) {
            return {
...
export async function handleDeleteCampaign(args, api) {
    try {
        const { campaign_id } = args;
        if (!campaign_id) {
            return {
...
export async function handleDebugCampaignAccess(args, api) {
    try {
...
export async function handleRenderCampaign(args, api) {
    try {
        ...
        const renderedData = rendered; // Use any to access response structure
