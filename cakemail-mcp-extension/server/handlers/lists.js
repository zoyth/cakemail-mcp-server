...
export async function handleListLists(args, api) {
    try {
        const { page, per_page, status, name, sort, order, with_count, account_id } = args;
        ...
        const detailedLists = await Promise.all((lists.data?.slice(0, 20) || []).map(async (list) => {
            try {
            ...
            content: [
                {
                    ...
                    text: `📋 **Contact Lists (${total} total)**\n\n` +
                        ...
                        (detailedLists?.map((list, i) => `${i + 1}. **${list.name}** (${list.id})\n` +
                            ...
                            `   🔄 Updated: ${list.updated_on || 'N/A'}`).join('\n\n') || 'No lists found.') +
...
export async function handleCreateList(args, api) {
    try {
        const { name, default_sender, language = 'en_US', redirections = {}, webhook = {}, account_id } = args;
        const normalizedAccountId = normalizeAccountId(account_id);
...
export async function handleGetList(args, api) {
    try {
...
export async function handleUpdateList(args, api) {
    try {
        const { list_id, name, default_sender, language, redirections, webhook, account_id } = args;
        const normalizedAccountId = normalizeAccountId(account_id);
        if (!list_id) {
            return {
                content: [{
        ...
        if (redirections !== undefined)
            updateData.redirections = redirections;
...
export async function handleDeleteList(args, api) {
    try {
        const { list_id, account_id } = args;
        const normalizedAccountId = normalizeAccountId(account_id);
        if (!list_id) {
...
export async function handleArchiveList(args, api) {
    try {
        const { list_id, account_id } = args;
        const normalizedAccountId = normalizeAccountId(account_id);
        if (!list_id) {
...
export async function handleGetListStats(args, api) {
    try {
        const { list_id, start_time, end_time, interval = 'day', account_id } = args;
        const normalizedAccountId = normalizeAccountId(account_id);
        if (!list_id) {
            return {
                content: [{
...
export async function handleGetListStatsTimeSeries(args, api) {
    try {
        const { list_id, start_time, end_time, interval = 'month' } = args;
        if (!list_id) {
            return {
                content: [{
                        type: 'text',
...
export async function handleGetListMovementLogs(args, api) {
    try {
        const { list_id, start_time, end_time, account_id } = args;
        if (!list_id) {
            return {
                content: [{
                        type: 'text',
        ...
        const monthlyMovements = new Map();
        ...
        allLogs.forEach((log) => {
            ...
            for (const [category, events] of Object.entries(movementEvents)) {
                if (events.includes(eventType)) {
                    if (category === 'subscribes')
                        month.subscribes++;
                    else if (category === 'unsubscribes')
                        month.unsubscribes++;
                    else if (category === 'bounces')
                        month.bounces++;
                    else if (category === 'spam_reports')
                        month.spam_reports++;
        ...
        if (sortedMonths.length > 0) {
            ...
            if (topEventTypes.length > 0) {
                content.push({
                    type: 'text',
                    text: `\n📋 **Top Event Types**\n` +
                        topEventTypes.map(([eventType, count]) => `• ${eventType}: ${count.toLocaleString()}`).join('\n')
                });
            }
        }
        else {
