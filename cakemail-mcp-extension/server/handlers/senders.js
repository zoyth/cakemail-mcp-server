...
export async function handleListConfirmedSenders(_args, api) {
    try {
            ...
            content: [{
                    ...
                    text: `✅ **Confirmed Senders (${confirmedSenders.length} total)**\n\n` +
                        (confirmedSenders.length > 0
                            ? confirmedSenders.map((sender, i) => `${i + 1}. **${sender.name}** <${sender.email}>\n` +
                                `   🆔 ID: ${sender.id}\n` +
                                `   🌐 Language: ${sender.language || 'en_US'}\n` +
                                `   ✅ Confirmed: Yes\n` +
                                `   📅 Created: ${sender.created_on || 'N/A'}`).join('\n\n')
