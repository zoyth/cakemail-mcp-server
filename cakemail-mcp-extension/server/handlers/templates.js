...
export async function handleListTemplates(args, api) {
    try {
        const { page = 1, per_page = 50, with_count = false, filter, sort = 'id', account_id } = args;
        const params = {
            page,
            per_page,
            with_count
...
export async function handleCreateTemplate(args, api) {
    try {
        const { name, description, content: templateContent, tags, account_id } = args;
        if (!name) {
        ...
        if (account_id)
            createData.account_id = account_id;
...
export async function handleGetTemplate(args, api) {
    try {
...
export async function handleUpdateTemplate(args, api) {
    try {
        const { template_id, name, description, content: templateContent, tags, account_id } = args;
        if (!template_id) {
            throw new Error('Template ID is required');
        }
        const updateData = {};
        ...
        if (templateContent)
            updateData.content = templateContent;
        if (tags && Array.isArray(tags))
...
export async function handleDeleteTemplate(args, api) {
    try {
        const { template_id, account_id } = args;
        if (!template_id) {
            throw new Error('Template ID is required');
...
export async function handleDuplicateTemplate(args, api) {
    try {
        const { template_id, new_name, new_description, account_id } = args;
        if (!template_id) {
            throw new Error('Template ID is required');
        }
        if (!new_name) {
        ...
        if (account_id)
            duplicateData.account_id = account_id;
...
export async function handleRenderTemplate(args, api) {
    try {
        const { template_id, account_id } = args;
        if (!template_id) {
            throw new Error('Template ID is required');
