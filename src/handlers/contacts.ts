...
// Define Contact type locally to fix implicit any types
...
export async function handleListContacts(args: any, api: CakemailAPI) {
  try {
    const { 
      list_id, page, per_page, email, status, sort, order, with_count, account_id 
    } = args;
    ...
    const contactData = contacts.data?.slice(0, 20).map((contact: ContactExtended) => ({
      id: contact.id,
      email: contact.email,
...
export async function handleImportContacts(args: any, api: CakemailAPI) {
  try {
      ...
      content: [{
        ...
        text: `🔍 **Contact Search Results**\n\n` +
              ...
              (contactData.slice(0, 10).map((contact, i) => 
                ...
                `   👤 Name: ${contact.first_name || ''} ${contact.last_name || ''}\n` +
                `   📊 Status: ${contact.status || 'N/A'}\n` +
                `   🏷️ Tags: ${(contact as any).tags?.join(', ') || 'None'}\n` +
                `   📅 Created: ${(contact as any).created_on || 'N/A'}`
