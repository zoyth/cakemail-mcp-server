...
import { mockCakemailResponses, createMockResponse, createTestScenarios } from '../mocks/index.js';
import { senderFixtures, createPaginatedResponse } from '../fixtures/index.js';
...
describe('Enhanced Sender Handlers with Rich Mock Data', () => {
  ...
  const validateEmail = (email: string): boolean => {
    return testDataSets.validation.validEmails.includes(email) ||
           /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
  };
